...
import { Pencil, Link as LinkIcon, Check, X, UploadCloud, FileText } from "lucide-react";
...
const MOCK_PROJECTS: Project[] = [
  { id: "p1", title: "NeuroForge Engine", shortInfo: "Advanced AI Agent orchestration layer.", detailedInfo: "Built with Node.js and TypeScript. Features robust queuing...", github: "https://github.com/srinivas/neuroforge" },
  { id: "p2", title: "Portfolio V2", shortInfo: "Next.js App Router cinematic shell.", detailedInfo: "Utilizes Framer Motion and SQLite for dynamic schema-less rendering.", github: "https://github.com/srinivas/portfolio-v2" },
  { id: "p3", title: "ArchAgent", shortInfo: "LLM code generation system.", detailedInfo: "Deep integration with Claude 3.5 Sonnet.", github: "" },
];
...
export default function ProjectsWorkspace() {
  ...
  const confirmEditing = () => {
    ...
    // Update local state
    setProjects(prev => prev.map(p => p.id === editState.id ? editState : p));
    
    // Stage global change
    addChange({
      id: `proj-${editState.id}`,
      section: "Projects",
                ...
                <div className="flex flex-col gap-6 p-8">
                  <div className="flex items-center justify-between border-b border-white/10 pb-4">
                    ...
                    <div className="flex items-center gap-2">
                      <motion.button whileTap={{ scale: 0.98 }} onClick={cancelEditing} className="flex h-8 w-8 items-center justify-center rounded bg-white/5 text-zinc-400 hover:bg-white/10 hover:text-white">
                        <X size={16} />
                      </motion.button>
                      <motion.button whileTap={{ scale: 0.98 }} onClick={confirmEditing} className="flex h-8 w-8 items-center justify-center rounded bg-red-500/20 text-red-500 hover:bg-red-500 hover:text-white transition-colors">
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                    <div className="flex flex-col gap-5">
                      ...
                      <div className="flex flex-col gap-2 flex-1">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Detailed Info (Master Readme)</label>
                        <textarea
                          value={editState.detailedInfo}
                          onChange={(e) => handleFieldChange("detailedInfo", e.target.value)}
                    ...
                    {/* PDF Staging Zone */}
                    <div className="flex flex-col gap-2">
                      <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Upload report in form of PDF</label>
                      <div className="flex h-full min-h-[200px] flex-col items-center justify-center rounded-xl border-2 border-dashed border-white/10 bg-white/[0.02] transition-colors hover:border-red-500/50 hover:bg-red-500/5 cursor-pointer">
